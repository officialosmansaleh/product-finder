...
def _df_filtered_subset(df: pd.DataFrame, filters: Dict[str, Any]) -> pd.DataFrame:
    ...
    for col in ["control_protocol", "interface", "insulation_class", "surge_common_mode", "surge_differential_mode", "emergency_present", "mounting_type", "shape", "housing_material", "housing_color", "product_family", "manufacturer"]:
        if col in filters:
...
def initialize_runtime_state() -> None:
    ...
    if USE_PRODUCT_DB and HAS_DATABASE:
        try:
            ...
            try:
                logger.info("Reading product DB table columns")
                columns = PRODUCT_DB._table_columns("products")
                logger.info("Product DB table columns read ok")
            except Exception:
                columns = []
            logger.info(
...
_COMPARE_FIELD_PRIORITY = [
    ...
    "control_protocol", "interface", "insulation_class", "surge_common_mode", "surge_differential_mode", "emergency_present",
